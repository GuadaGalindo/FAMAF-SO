...
static void write_dir_entry(fat_file parent, fat_dir_entry child_disk_entry,
                            u32 nentry) {
    fat_file child = init_file_from_dentry (child_disk_entry, parent); //obtenemos el file de la child_disk_entry
    if (fat_file_cmp_path (child, LOG_FILE) == 0){                     // comparamos si el path es el mismo que el log file
        child_disk_entry->base_name[0] = FAT_FILENAME_DELETED_CHAR;    // hacemos lo que pide   
...
void fat_file_remove(fat_file file, fat_file parent) {
    ...
    write_dir_entry(parent, file->dentry,file->pos_in_parent); // PREGUNTAR EN EL LAB
}
